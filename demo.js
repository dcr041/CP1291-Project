...
$(document).ready( async () => {
    ...
    try {
        ...
        const blogPostPromise = [
            getObject(`${domain}/posts/1`),
            getObject(`${domain}/posts/11`),
            getObject(`${domain}/posts/21`),
            getObject(`${domain}/posts/31`),
            getObject(`${domain}/posts/41`)
        ];

        const posts = await Promise.all(blogPostPromise);
